...
def group_by_tx_hash(
    ...
    #print('req_cols in group_by_tx_hash : ',req_cols)
    validate_existing_columns(df, req_cols)
    #print('cols after validate_existing_columns in  group_by_tx_hash : ', df.columns)
    # carry over labels if they are present in the data
    if label_col in df.columns:
        agg_list += [(label_col, ["max"])]

    #print('args of group_data = df' , '\n tx_id_col' , tx_id_col, '\n agg_list : ' ,agg_list )    
    result = group_data(
        df,
        tx_id_col,
        agg_list=agg_list,
    ...
    #print('cols after group_data in  group_by_tx_hash : ', df.columns)
    #print('cols result!!!! after group_data in  group_by_tx_hash : ', result.columns)
    ...
    #print('1 result.columns : ' , result.columns)
    result.columns = result.columns.str.replace(
        label_col + "_tx_max", label_col
    )
    #print('2 result.columns : ' , result.columns)
    # replace infs in the data to nan
    result.replace([np.inf, -np.inf], np.nan, inplace=True)
    #print('in group_by_tx_hash , result before return' , result.columns )
    return result
